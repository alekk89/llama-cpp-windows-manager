...
configuration. No ZIP configuration or GitHub App installation is needed for
this basic connector integration. See the
...
2. In SignPath, open **Users and Groups > CI builds**. Use its existing API token
...
3. The existing CI account is a submitter on both policies. Keep the token in the
...
4. Merge the workflow through the normal required PR checks, then dispatch it
   from `main`. The workflow itself also rejects other repositories and refs.
5. Inspect the completed request in SignPath: confirm the GitHub repository,
...
6. Provide SignPath support with the workflow run and signing-request links and
