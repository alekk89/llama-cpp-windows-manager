...
The workflow pins the signer certificate, requires Windows to report the
expected untrusted-root result for that self-signed certificate, requires a
timestamp, and checks that modifying the executable produces a hash mismatch.
It does not add the test certificate to any trusted root store. The test
artifacts are clearly labelled, expire after 14 days, and are never published
as GitHub release assets. Do not install the test certificate on users' machines
or describe these artifacts as publicly trusted.
