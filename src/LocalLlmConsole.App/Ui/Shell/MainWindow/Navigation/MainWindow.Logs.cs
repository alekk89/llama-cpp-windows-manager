...
public partial class MainWindow
{
    ...
    private async Task CreateDiagnosticsBundleAsync()
    {
        ...
        var wslProbe = await CaptureDiagnosticProbeAsync(
            "wsl-environment", "Microsoft WSL", DiagnosticErrorCodes.WslProbe,
            ...
            report => !report.WslExeFound ? "unsupported" : report.WslWorking ? "success" : "failure");
        var gpuProbe = await CaptureDiagnosticProbeAsync(
            "windows-gpu", "Windows hardware providers", DiagnosticErrorCodes.GpuProbe,
            () => _coreServices.App.GpuStatus.WindowsSummaryAsync(), "GPU probe unavailable");
        var cpuProbe = await CaptureDiagnosticProbeAsync(
            "windows-cpu", "Windows hardware providers", DiagnosticErrorCodes.CpuProbe,
            () => _coreServices.App.GpuStatus.CpuSummaryAsync(), "CPU probe unavailable");
        var result = await DiagnosticsBundleService.CreateAsync(new DiagnosticsBundleRequest(
            ...
            gpuProbe.Value,
            cpuProbe.Value,
            _diagnosticProbeHistory.Snapshot(),
            _sessions.DiagnosticEvents(),
            BuildAndUpdateDiagnosticsService.Capture(_infrastructureServices.AppUpdates)));
    ...
    private async Task<(T Value, DiagnosticProbeRecord Record)> CaptureDiagnosticProbeAsync<T>(
        string name,
        string provider,
        string errorCode,
        Func<Task<T>> action,
        T fallback,
        Func<T, string>? classify = null)
    {
        ...
        try
        {
            ...
            var classification = classify?.Invoke(value) ?? "success";
            var record = new DiagnosticProbeRecord(
                name, "1", provider, true, started, ended, classification, "success",
                classification == "failure" ? errorCode : "parsed", "", "");
            _diagnosticProbeHistory.Add(record);
            return (value, record);
        }
        catch (Exception ex)
        {
            var ended = DateTimeOffset.UtcNow;
            var record = new DiagnosticProbeRecord(
                name, "1", provider, true, started, ended,
                ex is TimeoutException or OperationCanceledException ? "timeout" : "failure",
                "exception", errorCode, "", $"{errorCode}:{ex.GetType().Name}");
            _diagnosticProbeHistory.Add(record);
            return (fallback, record);
        }
    }

    private async Task<(T Value, DiagnosticProbeRecord Record)> ObserveDiagnosticProbeAsync<T>(
        string name,
        string provider,
        string errorCode,
        Func<Task<T>> action,
