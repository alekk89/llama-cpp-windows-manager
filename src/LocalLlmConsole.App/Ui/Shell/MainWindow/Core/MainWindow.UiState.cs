...
public partial class MainWindow
{
    private void SetPage(string title, string subtitle)
    {
        ...
        ReleaseInactivePageView(_viewModel.CurrentPage);
        _viewModel.CurrentPage = title;
        SetActiveNavigation(title);
    }

    private void ReleaseInactivePageView(string page)
    {
        // Overview deliberately survives navigation so its dashboard history and
        // current hardware readings remain immediately available on return.
        if (string.Equals(page, "Overview", StringComparison.Ordinal)) return;

        PageHost.Content = null;
        switch (page)
        {
            case "Models":
                _modelsPage.ReleaseView();
                _downloadHistoryPageState.ShowSearch();
                break;
            case "Runtimes":
                _runtimesPage.ReleaseView();
                break;
            case "Benchmarks":
                ReleaseBenchmarksPage();
                break;
            case "Settings":
                _settingsPage.ReleaseView();
                break;
            case "Metrics":
                _lifetimePage.ReleaseView();
                break;
            case "Logs":
                _logsPage.ReleaseView();
                break;
            case "Windows":
                _windowsPage.ReleaseView();
                break;
            case "WSL Linux":
                _wslPage.ReleaseView();
                break;
        }
    }
