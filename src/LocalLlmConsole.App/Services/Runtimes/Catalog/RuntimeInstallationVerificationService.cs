...
public static class RuntimeInstallationVerificationService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        metadata["lastVerificationMessage"] = $"Local integrity baseline recorded for {files.Count} installed files.";
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var item in manifest.OfType<JsonObject>())
        {
            ...
            {
                problems.Add($"Duplicate manifest entry: {relative}");
                continue;
            }
        ...
        foreach (var installedFile in EnumerateInstallationFiles(folder))
        {
            cancellationToken.ThrowIfCancellationRequested();
            var relative = Path.GetRelativePath(folder, installedFile).Replace('\\', '/');
        ...
        var summary = status == RuntimeInstallationVerificationStatus.Verified
            ? $"Local integrity check matched {verified} installed files."
            : $"Local integrity check found {problems.Count} problem(s) after checking {verified} unchanged files.";
    ...
    public static RuntimeProvenance Describe(RuntimeRecord runtime)
    {
        ...
        var trustStatus = !managed
            ...
            : lastStatus.Equals("modified", StringComparison.OrdinalIgnoreCase)
                ...
                : lastStatus.Equals("missing", StringComparison.OrdinalIgnoreCase)
                    ...
                    : hasManifest ? "Local integrity checked" : "Managed runtime; re-verification unavailable";
        ...
        if (managed && hasManifest)
            detailLines.Add("Integrity scope: hashes are compared with the Manager's local installation baseline; this is change detection, not publisher authentication.");
    ...
    private static string NormalizeSha256(string? value)
        => Sha256Digest.NormalizeHex(value);
