...
""";
    ...
    {
        var create = LocalLlmConsole.ControlCli.Program.BuildRequestForTests(
        ...
        var assign = LocalLlmConsole.ControlCli.Program.BuildRequestForTests("groups", "assign", "qwen", "128K", "--group", "Batch");
        ...
        var unassign = LocalLlmConsole.ControlCli.Program.BuildRequestForTests("groups", "unassign", "qwen", "128K");
        Assert.Equal("DELETE", unassign.Method);
        Assert.Equal("/api/v1/models/qwen/profiles/128K/group", unassign.Path);
        Assert.Throws<InvalidOperationException>(() => LocalLlmConsole.ControlCli.Program.BuildRequestForTests(
