...
public sealed class RuntimeProcessLifecycleTests : ManagerRegressionTestBase
{
    [Fact]
    ...
    public void LlamaProcessSupervisorUsesCentralLogRedaction()
    {
        var source = File.ReadAllText(FindRepositoryFile(
            "src", "LocalLlmConsole.App", "Services", "Runtimes", "Sessions", "LlamaRuntimeOutputObserver.cs"));

        Assert.Contains("LogFileService.RedactSensitiveText", source, StringComparison.Ordinal);
        Assert.DoesNotContain("Regex.Replace", File.ReadAllText(FindRepositoryFile(
            "src", "LocalLlmConsole.App", "Services", "Runtimes", "Sessions", "LlamaProcessSupervisor.cs")), StringComparison.Ordinal);
    }
    ...
    [Fact]
    public async Task LlamaProcessSupervisorDistinguishesUnreadableEmbeddedMtpMetadataFromMissingCompanion()
    {
        using var supervisor = CreateTestLlamaSupervisor();
        var root = CreateTempRoot();
        ...
        Assert.DoesNotContain("No matching draft-mtp companion", error.Message, StringComparison.OrdinalIgnoreCase);
    }
    ...
    {
        var supervisorSource = File.ReadAllText(FindRepositoryFile(
            "src", "LocalLlmConsole.App", "Services", "Runtimes", "Sessions", "LlamaProcessSupervisor.cs"));
        ...
        Assert.DoesNotContain("Process.Start(", File.ReadAllText(FindRepositoryFile(
            "src", "LocalLlmConsole.App", "Services", "Runtimes", "Sessions", "LlamaProcessSupervisor.Wsl.cs")), StringComparison.Ordinal);
    }
