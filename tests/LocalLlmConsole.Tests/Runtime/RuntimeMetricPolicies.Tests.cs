...
public sealed class RuntimeMetricPoliciesTests : ManagerRegressionTestBase
{
    [Fact]
    public void RuntimeMetricRefreshCadenceAdaptsToVisibilityAndPage()
    {
        var now = DateTimeOffset.Parse("2026-08-26T12:00:00Z", System.Globalization.CultureInfo.InvariantCulture);

        Assert.Equal(TimeSpan.FromSeconds(1), RuntimeMetricRefreshCadencePolicy.Interval("Overview", minimizedOrHidden: false));
        Assert.Equal(TimeSpan.FromSeconds(5), RuntimeMetricRefreshCadencePolicy.Interval("Models", minimizedOrHidden: false));
        Assert.Equal(TimeSpan.FromSeconds(10), RuntimeMetricRefreshCadencePolicy.Interval("Overview", minimizedOrHidden: true));
        Assert.True(RuntimeMetricRefreshCadencePolicy.ShouldPoll("Overview", false, now, DateTimeOffset.MinValue));
        Assert.False(RuntimeMetricRefreshCadencePolicy.ShouldPoll("Models", false, now, now.AddSeconds(-4)));
        Assert.True(RuntimeMetricRefreshCadencePolicy.ShouldPoll("Models", false, now, now.AddSeconds(-5)));
        Assert.False(RuntimeMetricRefreshCadencePolicy.ShouldPoll("Overview", true, now, now.AddSeconds(-9)));
        Assert.True(RuntimeMetricRefreshCadencePolicy.ShouldPoll("Overview", true, now, now.AddSeconds(-10)));
    }
