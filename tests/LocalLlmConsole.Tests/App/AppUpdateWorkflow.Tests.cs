...
public sealed class AppUpdateWorkflowTests : ManagerRegressionTestBase
{
    [Fact]
    ...
    [Fact]
    public async Task InstalledUpdateNoticeCancellationPreservesNoticeForNextStartup()
    {
        ...
        await File.WriteAllTextAsync(
            noticePath,
            JsonSerializer.Serialize(new InstalledUpdateNotice("v2.5.0", "Release", "Notes", DateTimeOffset.UtcNow)),
            TestContext.Current.CancellationToken);
        using var cancelled = new CancellationTokenSource();
        cancelled.Cancel();

        await Assert.ThrowsAnyAsync<OperationCanceledException>(() =>
            AppUpdateService.TryConsumeInstalledNoticeAsync(root, cancelled.Token));
        Assert.True(File.Exists(noticePath));

        var notice = await AppUpdateService.TryConsumeInstalledNoticeAsync(root, TestContext.Current.CancellationToken);
        Assert.Equal("v2.5.0", notice?.Version);
        Assert.False(File.Exists(noticePath));
    }
