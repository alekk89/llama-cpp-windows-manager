...
public sealed class InfrastructureLifecycleTests : ManagerRegressionTestBase
{
    [Fact]
...
""";
    ...
    public void LocalAppServiceObservesRequestHandlerTasks()
    {
        ...
        var tracker = File.ReadAllText(FindRepositoryFile("src", "LocalLlmConsole.App", "Services", "Infrastructure", "HttpListenerRequestTracker.cs"));
        var acceptLoop = File.ReadAllText(FindRepositoryFile("src", "LocalLlmConsole.App", "Services", "Infrastructure", "HttpListenerAcceptLoop.cs"));

        Assert.Contains("HttpListenerAcceptLoop.RunAsync", source, StringComparison.Ordinal);
        Assert.Contains("_requestTracker.Track", source, StringComparison.Ordinal);
        Assert.Contains("AbortAndDrainAsync", source, StringComparison.Ordinal);
        Assert.Contains("ObserveCompletionAsync", tracker, StringComparison.Ordinal);
        Assert.Contains("_tasks", tracker, StringComparison.Ordinal);
        Assert.Contains("LastListenerError", source, StringComparison.Ordinal);
        Assert.Contains("queueRequest(context, cancellationToken)", acceptLoop, StringComparison.Ordinal);
        Assert.Contains("MaximumConsecutiveErrors", acceptLoop, StringComparison.Ordinal);
        Assert.Contains("await Task.Delay(retryDelay, cancellationToken)", acceptLoop, StringComparison.Ordinal);
        Assert.Contains("stopListener();", acceptLoop, StringComparison.Ordinal);
        Assert.DoesNotContain("_ = Task.Run(() => HandleAsync", source, StringComparison.Ordinal);
    }

    [Fact]
    public async Task HttpListenerAcceptLoopStopsAndFaultsAfterRepeatedAcceptFailures()
    {
        var attempts = 0;
        ...
        var failure = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            HttpListenerAcceptLoop.RunCoreAsync<object>(
                () =>
                {
                    attempts++;
                    return Task.FromException<object>(new IOException($"accept-{attempts}"));
                },
                () => listening,
                () =>
                {
                ...
                (_, _) => throw new InvalidOperationException("No request should be queued."),
                errors.Add,
                TimeSpan.Zero,
    ...
    [Fact]
    public async Task BoundedTaskDrainReturnsAfterTimeoutAndObservesLateCompletion()
    {
        ...
        stopwatch.Stop();
        completion.TrySetException(new InvalidOperationException("observed"));
        await Task.Yield();
    ...
    [Fact]
    public async Task BackgroundTaskApplicationServiceDrainsRegisteredWorkAndRejectsNewWork()
    {
        var service = new BackgroundTaskApplicationService();
        var started = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var release = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var calls = new List<string>();
        var actions = new BackgroundTaskApplicationActions(_ => { }, _ => Task.CompletedTask);
        var running = service.RunAsync(async () =>
        {
