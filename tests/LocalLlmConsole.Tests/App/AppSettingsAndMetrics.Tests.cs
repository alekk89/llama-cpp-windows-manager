...
public sealed class AppSettingsAndMetricsTests : ManagerRegressionTestBase
{
    ...
    public async Task LifetimeMetricResetApplicationServiceOwnsResetBranches()
    {
        ...
        var total = new LifetimeMetricRow
        {
            Kind = LifetimeMetricRowKind.Total,
            ModelName = "All models"
        };
        var blocked = new LifetimeMetricRow
        {
            ModelName = "Blocked",
            ModelId = "blocked",
            CanReset = false
        };
        var missingModelId = new LifetimeMetricRow
        {
            ModelName = "Missing"
        };
        var model = new LifetimeMetricRow
        {
            ModelName = "Model One",
            ModelId = "model-1"
        };
    ...
    public async Task LifetimeMetricsApplicationServiceOwnsTokenUsagePersistence()
    {
        ...
        await service.AddUsageAsync(new TokenUsageDelta(
            "model-b",
            "Model B",
            0,
            0,
        ...
        var requestBucket = Assert.Single(
            await store.ListTokenUsageBucketsAsync(),
            bucket => bucket.RequestCount > 0);
        Assert.Equal(2, requestBucket.RequestCount);
        Assert.Equal(1, requestBucket.FailedRequestCount);
        ...
        Assert.Equal(5, service.DataVersion);
