...
public sealed partial class ReleaseHardeningTests
{
    ...
    [Fact]
    public void OverviewDashboardMetricRowsUseDirectVerticalDragReordering()
    {
        var card = File.ReadAllText(FindRepositoryFile("src", "LocalLlmConsole.App", "Ui", "Pages", "Overview",
            "OverviewDashboardCardView.Reorder.cs"));
        var row = File.ReadAllText(FindRepositoryFile("src", "LocalLlmConsole.App", "Ui", "Pages", "Overview",
            "OverviewDashboardMetricRowView.cs"));

        Assert.Contains("_values.PreviewMouseLeftButtonDown += BeginMetricDrag", card, StringComparison.Ordinal);
        Assert.Contains("SystemParameters.MinimumVerticalDragDistance", card, StringComparison.Ordinal);
        Assert.Contains("MetricDropIndex(pointer.Y, row.Root)", card, StringComparison.Ordinal);
        Assert.Contains("Cursors.SizeNS", row, StringComparison.Ordinal);
        Assert.DoesNotContain("ReorderButton", row, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void SettingsAndControlCompatibilityMutateTheCanonicalDashboardLayout()
    {
