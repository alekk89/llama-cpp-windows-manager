...
public sealed class ReleaseGateIntegrationTests : ManagerRegressionTestBase
{
    [Fact]
    public async Task LocalControlApiRoutesCoverInventorySettingsValidationAndOperations()
    {
        ...
        sessions.AttachExisting(
            ...
            "session-api-model",
            DateTimeOffset.UtcNow);
        var inferredSelf = JsonSerializer.SerializeToNode((await api.HandleAsync(Request("GET", "/api/v1/self"))).Body)!.AsObject();
        Assert.False(inferredSelf["identified"]!.GetValue<bool>());
        Assert.Equal("inferred-single-running-session", inferredSelf["confidence"]!.GetValue<string>());
        Assert.Single(inferredSelf["candidates"]!.AsArray());
        var exactSelf = JsonSerializer.SerializeToNode((await api.HandleAsync(Request(
            "GET",
            "/api/v1/self",
            query: new Dictionary<string, string> { ["sessionId"] = "session-api-model" }))).Body)!.AsObject();
        Assert.True(exactSelf["identified"]!.GetValue<bool>());
        Assert.Equal("exact", exactSelf["confidence"]!.GetValue<string>());
    ...
    public async Task SettingsWorkflowsAndLocalHttpHostCoverReleaseSecurityPaths()
    {
        ...
        var values = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["benchmarkPreventSystemSleep"] = "No",
            ["benchmarkStopActiveSessions"] = "Yes",
        ...
        Assert.False(updated.Settings.ShowOverviewModelSection);
        ...
        Assert.False(updated.Settings.BenchmarkPreventSystemSleep);
        Assert.True(updated.Settings.BenchmarkStopActiveSessions);
        ...
        var benchmarkSettings = new SettingsPageDefinitionService().BuildRows(updated.Settings)
            .Where(row => row.Group == Loc.T("Settings.Group.Benchmarks"))
            .ToArray();
        Assert.Equal(2, benchmarkSettings.Length);
        Assert.Equal("No", benchmarkSettings.Single(row => row.Key == "benchmarkPreventSystemSleep").Value);
        Assert.Equal("Yes", benchmarkSettings.Single(row => row.Key == "benchmarkStopActiveSessions").Value);
    ...
    public void PageViewModelsRenderDomainStateAndEdgeCases()
    {
        ...
        Assert.Contains("Loading", overview.SessionRows[1].State, StringComparison.Ordinal);
        ...
        Assert.Contains("boom", overview.SessionRows[0].State, StringComparison.Ordinal);
        ...
        Assert.Equal("a_metric", metrics.Rows[0].Name);
        ...
        Assert.Equal("15", lifetime.Rows[0].OutputTokens);
        Assert.Equal("25", lifetime.Rows[0].TotalTokens);
        ...
        Assert.Equal("No Linux distro detected", wsl.Rows[0].Name);
        ...
        Assert.Equal("Selected", wsl.Rows[0].ActionLabel);
        ...
        Assert.Contains("Vision + mmproj", hf.SearchRows[0].Signals, StringComparison.Ordinal);
        ...
        Assert.Equal(JobStatus.Paused.ToString(), downloadRow.Status);
        ...
        var logInfo = new FileInfo(runtimeLog);
        logs.ReplaceLogs(
            [new LogPageFile(logInfo.FullName, logInfo.Name, logInfo.LastWriteTime, logInfo.LastWriteTimeUtc, logInfo.Length)],
            new Dictionary<string, JobRecord>(), runtimeLog, model.Name);
